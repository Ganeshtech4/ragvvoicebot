...
export function ChatShell() {
  ...
  return (
    <>
      ...
      {isVoiceOpen && session?.user && (
        <VoiceInterface
          tenantId={session.user.tenantId || "tenant-tech"}
          ...
          onAssistantText={(text, isFinal) => {
            setMessages((prev) => {
              ...
              if (last && last.role === "assistant" && last.id.startsWith("voice-ast-")) {
                const lastText = (last as any).content || last.parts?.filter((p: any) => p.type === "text").map((p: any) => p.text).join("") || "";
                return [
                  ...prev.slice(0, -1),
                  {
                    ...last,
                    content: isFinal ? text : lastText + text,
                    parts: [{ type: "text", text: isFinal ? text : lastText + text }],
                  } as any,
                ];
              } else {
                return [
